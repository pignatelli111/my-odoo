...
- **Non ancora:** import Excel completo RDA/ACO/ACP dal file consulente; Logikal non sostituisce conferma finale.

**Come verificare**  
1. Preventivo vinto → commessa → RDA da distinta.  
2. Distinta ITEM: righe vetro/zanzariere → controllare MQ calcolati e flag tecnico.  
3. RDA → stati **Dati tecnici** → passare a **Pronto per RFQ/PO** solo dopo revisione.  
4. Tentare RFQ senza passaggio → deve comparire messaggio di blocco.

📖 [COSIMO_PUNTO1_DATI_TECNICI.md](COSIMO_PUNTO1_DATI_TECNICI.md) — Modulo: `sbu_estimate`, `sbu_purchase_flow`
...
|---|-----------|-------|-----------------|
| 1 | Dati tecnici vs ANACO / PO | ⚠️ | Fasi RDA + regole vetro/zanzariere |
...
1. **Upgrade moduli SBU** su database produzione dopo ogni deploy.  
2. **UAT a campione** su una commessa reale: punti 1→2→5→11→10→13.  
3. **Logikal** e **import Excel tecnico** (punto 1a): pianificare con file campione e licenza ERP Logikal.  
