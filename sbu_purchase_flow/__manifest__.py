{
    'name': 'SBU Purchase Flow',
    'version': '19.0.1.0.101',
    'summary': 'RDA/ACP/ACO/LDS purchase requests; full TMS Excel origin integration',
    'author': 'SBU Development',
    'category': 'Purchase',
    'depends': [
        'base',
        'mail',
        'purchase',
        'account',
        'stock',
        'project',
        'sbu_estimate',
        'sbu_stock_config',
    ],
    'data': [
        'security/sbu_purchase_security.xml',
        'security/ir.model.access.csv',
        'data/sbu_purchase_request_sequence.xml',
        'data/sbu_delivery_standard_data.xml',
        'data/sbu_delivery_standard_supplement.xml',
        'data/sbu_workflow_route_data.xml',
        'data/sbu_workflow_route_elenco_data.xml',
        'wizards/sbu_purchase_request_line_bulk_wizard_views.xml',
        'wizards/sbu_purchase_request_create_wizard_views.xml',
        'wizards/sbu_purchase_request_excel_import_wizard_views.xml',
        'wizards/sbu_purchase_request_bom_import_wizard_views.xml',
        'views/sbu_purchase_request_views.xml',
        'views/sbu_purchase_request_line_views.xml',
        'views/sbu_purchase_request_offer_views.xml',
        'views/purchase_order_views.xml',
        'views/project_project_views.xml',
        'views/sbu_project_budget_views.xml',
        'views/sbu_delivery_standard_views.xml',
        'views/sbu_workflow_route_views.xml',
        'views/sbu_tms_origin_views.xml',
        'views/sbu_estimate_views.xml',
        'views/sbu_estimate_budget_views.xml',
        'views/menu.xml',
    ],
    'external_dependencies': {
        'python': ['openpyxl'],
    },
    'installable': True,
    'application': False,
    'license': 'LGPL-3',
}
