...
class TestSbuPrQtyResidual(TransactionCase):
    ...
    def test_demand_loss_shows_103_not_uom(self):
        ...
        self.assertIn('1.03', line.qty_demand_hint or '')
        self.assertIn('3', line.qty_demand_hint or '')
    ...
    def test_partial_rfq_updates_remaining(self):
        ...
        line.invalidate_recordset(['qty_ordered', 'qty_remaining'])
        self.assertAlmostEqual(line.qty_ordered, 3.0, places=2)
        self.assertAlmostEqual(line.qty_remaining, 7.0, places=2)
    ...
    def test_second_rfq_uses_remaining_only(self):
        ...
        po1.order_line.product_qty = 4.0
        line.invalidate_recordset(['qty_remaining'])
        self.assertAlmostEqual(line.qty_remaining, 6.0, places=2)
        ...
        line.invalidate_recordset(['qty_ordered'])
    ...
    def test_create_rfq_reuses_draft_po_and_residual(self):
        ...
        pol.product_qty = 3.0
        line.invalidate_recordset(['qty_remaining'])
        self.assertAlmostEqual(line.qty_remaining, 7.03, places=2)
        pr.action_create_rfq()
        self.assertEqual(len(pr.purchase_order_ids), 1)
        pol.invalidate_recordset(['product_qty'])
