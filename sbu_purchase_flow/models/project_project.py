...
class ProjectProject(models.Model):
    ...
    sbu_lds_entry_ids = fields.One2many(
        'sbu.lds.entry',
        'project_id',
        string='LDS register',
    )
    ...
    def action_view_sbu_lds_entries(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('LDS register'),
            'res_model': 'sbu.lds.entry',
            'view_mode': 'list,form',
            'domain': [('project_id', '=', self.id)],
            'context': {'default_project_id': self.id},
        }

    def action_view_sbu_drawing_register(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('Drawing register'),
            'res_model': 'sbu.drawing.register',
            'view_mode': 'list,form',
            'domain': [('project_id', '=', self.id)],
            'context': {'default_project_id': self.id},
        }
